...
int communicate_server(char* file_name, SOCKET* p_socket) {
    ...
    char* parsed_message    = (char*)malloc(sizeof(char) * BUFFER_SIZE_BYTES * BITS_IN_BYTE + 1);

    // each iteration is per packet
    while (TRUE) {

        recv_result = recv_packet(messageBuffer, BUFFER_SIZE_BYTES, p_socket, &packet_size);
        ...
        get_bits(messageBuffer, parsed_message, packet_size);
...
/// <returns>zero if successful, one otherwise</returns>
int parse_packet(FILE* p_file, char* source, int packet_size) {

    int total_bits = packet_size * BITS_IN_BYTE;
    int blocks = total_bits / BITS_IN_BLOCK_WITH_HAMMING;
    char parsed_block[DATA_BITS_IN_BLOCK];
    int temp = 0;
...
void decode_hamming(char* encoded_buffer, char* decoded_buffer) {
    int error_index = 0;
    int i = 0;
    for (i = 0; i < BITS_IN_BLOCK_WITH_HAMMING; i++) {
        if (((int)encoded_buffer[i]) == 1)
            error_index ^= (i + 1);
    }
    ...
    int j = 2;
    for (j = 2; j < BITS_IN_BLOCK_WITH_HAMMING; j++) {
        if (ceil(log2((double)j + 1)) == floor(log2((double)j + 1)))
            continue;
        decoded_buffer[decoded_buffer_index] = encoded_buffer[j];
...
/// <param name="p_file">pointer to file</param>
/// <returns>zero if successful, one otherwise</returns>
void get_bits(char* read_target, char* data_buffer, int packet_size) {
    ...
    for (int i = 0; i < packet_size; i++) {
        ...
        for (int j = 0; j < BITS_IN_BYTE; j++) {
            temp = read_mask & (int)read_target[i];
            if (temp > 0) {
                data_buffer[ref_index + j] = (int)1;
            }
            else
                data_buffer[ref_index + j] = (int)0;
            read_mask >>= 1;
        }
    }
}
